...
export async function listRecentQuizAttempts(userId: string, maxCount = 50) {
  if (!userId) return [];
...
export async function listRecentExamAttempts(userId: string, maxCount = 25) {
  if (!userId) return [];
...
export async function listRecentExamResults(userId: string, maxCount = 10) {
  if (!userId) return [];
