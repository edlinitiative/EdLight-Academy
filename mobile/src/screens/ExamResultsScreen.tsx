  ...
  return (
    <View style={{ marginBottom: 12 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
        <Text style={{ fontSize: 13, fontWeight: '600', color: '#374151', flex: 1 }} numberOfLines={1}>{section === 'Général' ? t('Général', 'Jeneral') : section}</Text>
      ...
      <Text style={{ fontSize: 11, color: '#94a3b8', marginTop: 3 }}>{correct}/{total} {t('correctes', 'kòrèk')}</Text>
    </View>
  );
}
        ...
        {questions.length > 0 && (
          <View className="mx-4 mt-5">
            <View className="flex-row items-center justify-between mb-3">
              <Text className="text-base font-bold text-gray-900">{t('Revue des questions', 'Revi kesyon yo')}</Text>
              <Text className="text-xs text-gray-400">{questions.length} {t('questions', 'kesyon')}</Text>
            </View>
            ...
            {filteredQuestions.length === 0 ? (
              <View className="items-center py-6">
                <Text className="text-gray-400 text-sm">{t('Aucune question dans ce filtre.', 'Pa gen kesyon nan filt sa a.')}</Text>
              </View>
            ) : (
